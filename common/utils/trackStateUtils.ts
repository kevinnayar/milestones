...
export function getReorderedTrackState(trackState: TrackState): TrackState {
...
function instantiateTrackState(payload: StartActionPayload): TrackState {
  const { startDate, template, version } = payload;
  ...
  const trackState = getResolvedTrackState(startDate, milestonesWithStart);
  return trackState;
}
...
export function trackStateReducer(action: TrackAction, state: void | TrackState): TrackState {
  ...
  switch (action.type) {
    ...
    case 'ADD': {
      ...
      return getReorderedTrackState(newState);
    }
    ...
    case 'REMOVE': {
      ...
      if (found) {
        const ids = trackState.ids.filter((id) => id !== action.payload.milestoneId);
        // eslint-disable-next-line no-unused-vars, @typescript-eslint/no-unused-vars
        const { [action.payload.milestoneId]: _ignore, ...idMap } = trackState.idMap;
    ...
    case 'INCOMPLETE': {
      ...
      const found = trackState.idMap[milestoneId];
